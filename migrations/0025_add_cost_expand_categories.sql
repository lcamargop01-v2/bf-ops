-- Migration 0025: Add cost column to products, remove restrictive category CHECK constraint
-- The CHECK constraint on category is too restrictive (22 values) vs spreadsheet (35+ categories)
-- SQLite cannot ALTER CHECK constraints, so we must recreate the table

-- Step 1: Create new products table without the CHECK constraint on category
...
-- Step 2: Copy data from old table to new (cost defaults to 0)
...
-- Step 4: Recreate any indexes on products
